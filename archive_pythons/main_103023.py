...
# Define a dictionary to keep track of row selection counts
row_selection_counts = {}
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        # Connect the item selection change event to the on_change function
    ...
    def on_change(self):
        ...
        global now
        global now2
        selected_rows = [item.text() for item in self.listWidget.selectedItems()]
        for row in selected_rows:
            row_index = self.listWidget.findItems(row, Qt.MatchExactly)[0].row()
            row_selection_counts[row_index] = row_selection_counts.get(row_index, 0) + 1
        print("Row Selection Counts:", row_selection_counts)
